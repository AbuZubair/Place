   var loket = $('#loket_hidden').val();
   var type = $('#tipe_hidden').val();

   /*submit sesion for loket*/
  $('#btn_submit_sess_loket').click(function (e) {  

    var select_loket = $('#select_loket').val();
    if( select_loket == "" ){
      alert('Silahkan pilih loket !'); return false;
    }
    var select_tipe = $('#select_tipe').val();
    if( select_tipe == "" ){
      alert('Silahkan pilih tipe loket !'); return false;
    }
    ...
    e.preventDefault();    
      ...
      $.ajax({
        url:"counter/process_save_session_counter",
        data:{data:data},
        dataType: "json", 
        type:"POST",       
        success:function (data) {
          console.log(data);
          if( data.success==1){
            $('#form_session_antrian').hide();
            $('#counter_apps').show();
            $('#loket_hidden').val( data.sess_counter_data.loket);
            $('#label_loket').text( data.sess_counter_data.loket);
            $('#tipe_hidden').val( data.sess_counter_data.tipe);

            $('#label_tipe').text( data.sess_counter_data.tipe.toUpperCase());
            
            format_no = pad(data.counter, 3);
            $('#counter_number').text( data.tipe+''+format_no );
            $('#counter_number_value').val( data.counter );
            /*info antrian*/
            $('#from_num').text(data.counter);
            $('#to_num').text(data.counter);

            getMenu('registration/reg_klinik/index');
          }
